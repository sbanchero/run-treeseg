...
query_exp = "select edgelength, resolution, percentil, zmin, zmax, smooth, dmin, dmax from downsample, getdemslice, findstems LIMIT 10"
...
def main(args):	
	...
	cursor = con.cursor()
	id_exp = 0
	for edgelength, resolution, percentil, zmin, zmax, smooth, dmin, dmax in cursor.execute(query_exp):
		print(f"ID: {id_exp}")
		...
		run_findstems(smooth, dmin, dmax)
